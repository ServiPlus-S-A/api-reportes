...
describe('ReportesService', () => {
  ...
  const mockFinanzasData = [
    { id: '1', descripcion: 'Ingreso 1', monto: 1000, tipo: 'ingreso', fecha: '2026-05-01' },
    { id: '2', descripcion: 'Ingreso 2', monto: 2000, tipo: 'ingreso', fecha: '2026-05-02' },
    { id: '3', descripcion: 'Egreso 1', monto: 500, tipo: 'egreso', fecha: '2026-05-03' },
  ];
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ReportesService,
        {
          ...
          useValue: {
            fetchIngresosPorPeriodo: jest.fn().mockResolvedValue(mockFinanzasData),
          },
        },
        {
    ...
    firebaseRepository = module.get<FirebaseReporteRepository>(FirebaseReporteRepository);
  });
  ...
  describe('generarReporte - Cache Miss', () => {
    it('should fetch fresh data, perform correct calculations, write to cache, and save audit logs', async () => {
      ...
      expect(finanzasAdapter.fetchIngresosPorPeriodo).toHaveBeenCalledWith('2026-05');
  ...
  describe('generarReporte - Cache Hit', () => {
    it('should return cached data directly and write audit logs', async () => {
      ...
      const mockRedisInstance = (service as any).redisClient;
      mockRedisInstance.get = jest.fn().mockResolvedValue(JSON.stringify(cachedReport));
